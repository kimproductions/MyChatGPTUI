...
export function loadConversationHistory(fileName) {
    let storedItem = localStorage.getItem(fileName);

    // if (storedItem === 'undefined' || storedItem === null || storedItem == '[]') {
    //   storedItem = '[]';
    // }
    // console.log(storedItem);

    let storedConversation = JSON.parse(storedItem);
    // console.log('stored convo: ' + typeof(storedConversation));

    let conversationList = storedConversation ? storedConversation : [{ conversationName: "New Conversation", conversation: [] }];
    console.log(conversationList);
    return conversationList;
}

export function saveConversationHistory(fileName, conversationList) {
    const controls = document.querySelector("#controls");

    if (conversationList[0]) {
        localStorage.setItem(fileName, JSON.stringify(conversationList));
        controls.classList.remove("ghosted");
    }
    else {
        conversationList = [{ conversationName: "New Conversation", conversation: [] }];
        localStorage.setItem(fileName, JSON.stringify(conversationList));
        controls.classList.add("ghosted");
    }
}
